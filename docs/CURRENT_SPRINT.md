...
| 8.5 | QA-012 (caché de consultas "relacionados"), QA-016 (script inline de metabox sin dependencia formal), QA-035 (autoplay de testimonios sin pausa accesible), QA-038 (`<link rel="canonical">` ausente) | 🟡 **Integración de código completa** (`inc/helpers.php`, `inc/meta-boxes.php`, `inc/enqueue.php`, `inc/seo.php`, `assets/js/main.js`, `assets/css/main.css`, `assets/js/admin-proyecto-gallery.js` nuevo) — **pendiente únicamente de tus pruebas funcionales reales** antes de aprobar. Ver `docs/DECISIONS.md` D-102. |
...
El Entregable 8.5 (QA-012, QA-016, QA-035, QA-038) tiene ya la **integración de código completa** en los 7 archivos que le correspondían (`docs/DECISIONS.md` D-102). Falta que el usuario ejecute las pruebas funcionales reales listadas en esa misma entrada (relacionados de Servicio/Proyecto siguen funcionando y se actualizan al publicar/editar contenido; selector de galería de Proyecto sigue funcionando; botón de pausa del slider de testimonios accesible por teclado; `<link rel="canonical">` correcto en Home/Servicio/archivo/paginación/búsqueda) antes de aprobarlo, siguiendo el mismo patrón ya usado en el 8.3 (D-096 → D-097) y el 8.4 (D-098/D-099 → D-101). Con esa aprobación, el Sprint 8 continuaría con el Entregable 8.6 (QA-036), siguiente en la reorganización de D-043.
