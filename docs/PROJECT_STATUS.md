...
**Última actualización (integración de código completa del Entregable 8.5):** Sprint 7 COMPLETADO. Sprint UX-11 CERRADO Y APROBADO EN SU TOTALIDAD (`DECISIONS.md` D-083 a D-094). **Sprint 8 ("Cierre de Hallazgos QA") EN CURSO: Entregables 8.1, 8.2 (QA-030), 8.3 (QA-031) y 8.4 (QA-032, QA-033, QA-034) aprobados explícitamente por el usuario** (el 8.3 con aprobación final tras 4 pruebas funcionales reales en Apache/2.4.68 + PHP 8.2.33, `DECISIONS.md` D-096/D-097; el 8.4 tras pruebas funcionales reales confirmadas por el usuario sin desglose caso por caso, `DECISIONS.md` D-098/D-099/D-101). **Entregable 8.5 (QA-012, QA-016, QA-035, QA-038) con integración de código completa** en 7 archivos (`inc/helpers.php`, `inc/meta-boxes.php`, `inc/enqueue.php`, `inc/seo.php`, `assets/js/main.js`, `assets/css/main.css`, `assets/js/admin-proyecto-gallery.js` nuevo — `DECISIONS.md` D-102), pendiente únicamente de pruebas funcionales reales antes de aprobarse. Entregables 8.6/8.7 reorganizados por prioridad/dependencias/riesgo, sin iniciar (`DECISIONS.md` D-043). QA-041 verificado y cerrado (`page.php` no existía al momento de la verificación; fue creado después, en Sprint UX-6).
...
| # | Módulo | Archivos | Estado |
|---|--------|----------|--------|
| 31 | Entregable 8.5 — QA-012 (caché "relacionados"), QA-016 (script inline de metabox), QA-035 (autoplay accesible), QA-038 (canonical) | `inc/helpers.php`, `inc/meta-boxes.php`, `inc/enqueue.php`, `inc/seo.php`, `assets/js/main.js`, `assets/css/main.css`, `assets/js/admin-proyecto-gallery.js` (nuevo) | 🟡 **Integración de código completa**, pendiente únicamente de pruebas funcionales reales (`DECISIONS.md` D-102) |

## 4. Módulos pendientes

| # | Módulo | Prioridad |
|---|--------|-----------|
| 32 | Entregable 8.6 — QA-036 (accesibilidad: gestión de foco en overlays) — requiere decisión de diseño | Media |
...
| QA-012, QA-016, QA-035, QA-038: hallazgos Medios aislados | 🟡 Media | Entregable 8.5, **integración de código completa** (`docs/DECISIONS.md` D-102); pendiente únicamente de pruebas funcionales reales antes de aprobar. |
...
Con la fase UX cerrada en su totalidad (Sprints UX-1 a UX-11) y con los Entregables 8.1, 8.2, 8.3 y 8.4 del Sprint 8 ya **aprobados** (`DECISIONS.md` D-095, D-096/D-097, D-098/D-099/D-101), el **Entregable 8.5** (QA-012, QA-016, QA-035, QA-038) tiene su **integración de código completa** (`DECISIONS.md` D-102) y solo espera las pruebas funcionales reales del usuario para poder aprobarse. El siguiente candidato tras 8.5 es el Entregable 8.6 (QA-036).
