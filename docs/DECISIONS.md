...
### D-034 — Política de actualización incremental de documentación + nuevo documento `CURRENT_SPRINT.md`
- **Fecha:** Tras el Entregable 6B.2
- **Problema:** La regla original (D-030) exigía actualizar los 6-7 documentos de control al cierre de cada Entregable, incluso cuando varios de ellos no tenían ningún cambio real que registrar (ej. `QA_REPORT.md` cuando ningún hallazgo cambia, o `PROJECT_STATUS.md` cuando el estado del Sprint no varía). Esto generaba desgaste de contexto/mensajes sin aportar valor, y `HANDOFF.md` en particular se volvía voluminoso al actualizarse en cada Entregable en vez de reservarse para transiciones reales entre sesiones/Sprints.
- **Solución elegida:**
  1. Cada documento de control se actualiza **únicamente** cuando cambia lo que ese documento específico existe para registrar (criterios explícitos por documento: `PROJECT_STATUS.md` solo si cambia el estado del proyecto/Sprint/Entregable; `TODO.md` solo si se completa un Entregable o cambia el backlog; `CHANGELOG.md` solo si hay cambio de código, documentación relevante, o nueva versión; `TREE.md` solo si cambia la estructura de archivos/carpetas; `ARCHITECTURE.md` solo si cambia la arquitectura, flujo, dependencias, organización, o la metodología permanente; `DECISIONS.md` solo ante una nueva decisión formal; `QA_REPORT.md` solo si cambia un hallazgo).
  2. `HANDOFF.md` deja de actualizarse en cada Entregable — solo se actualiza al finalizar un Sprint completo, al cerrar una sesión que pueda continuarse después, o ante un cambio importante de arquitectura.
  3. Se crea `CURRENT_SPRINT.md`, documento nuevo y permanente, pequeño y preciso, que se actualiza **en cada Entregable** y absorbe el detalle operativo de corto plazo (Entregables del Sprint activo, archivos creados/modificados, próximo Entregable, riesgos/observaciones puntuales) que antes recaía parcialmente en `HANDOFF.md`.
- **Alternativas descartadas:** Mantener la actualización obligatoria de todos los documentos en cada Entregable "por seguridad" (rechazado: es exactamente el patrón que esta decisión busca corregir — desgaste de contexto sin beneficio real, ya observado en varios cierres de Entregable anteriores donde documentos como `QA_REPORT.md` se re-guardaban sin ningún cambio de contenido).
- **Motivo:** Optimizar el consumo de contexto/mensajes por Entregable (alineado con el espíritu original de D-030: "minimizar el consumo de contexto y mensajes, pero nunca limitar la arquitectura ni la calidad del código"), sin perder trazabilidad — `CURRENT_SPRINT.md` cubre la continuidad de corto plazo, y `HANDOFF.md` sigue siendo el documento maestro de transferencia entre conversaciones.
- **Impacto:** A partir de este punto, antes de editar cualquier documento de control al cierre de un Entregable, se verifica explícitamente si ese documento tiene algo real que registrar; si no, se declara expresamente "sin cambios" en vez de tocarlo. `HANDOFF.md` se vuelve más estable entre Entregables de un mismo Sprint, actualizándose solo en las 3 situaciones ya descritas.
