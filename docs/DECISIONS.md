...
- **Impacto:** Todos los Entregables entregados a partir de esta decisión (incluyendo, retroactivamente, 7.1 y 7.2) se marcan como **"Entregado — pendiente de aprobación"** en `PROJECT_STATUS.md`/`TODO.md`/`TREE.md`/`CURRENT_SPRINT.md` hasta recibir confirmación explícita del usuario, momento en el cual pasan a **"Completado"**. `HANDOFF.md` sección 16 se actualiza con esta regla como parte del flujo obligatorio de cierre.

---

### D-039 — Corrección QA-018: responsive de `.ce-header__top` vía `flex-wrap` + layout apilado
...
- **Solución elegida:** Se aplicó la Alternativa B propuesta en el propio hallazgo (`QA_REPORT.md`): una regla `@media (max-width: 767.98px)` que apila `.ce-header__top .ce-container` (`flex-direction: column`) y envuelve `.ce-header__contact`/`.ce-header__social` (`flex-wrap: wrap`), añadida como sección 24 (nueva, al final) de `assets/css/main.css`.
- **Alternativas descartadas:** Ocultar `.ce-header__top` por debajo de 768px y mover esa información al menú móvil off-canvas (Alternativa A del hallazgo original) — descartada porque habría requerido modificar el markup de `header.php` (el menú móvil actual no incluye teléfono/correo/horario), excediendo el alcance de una corrección puramente CSS y el alcance aprobado explícitamente por el cliente para este Entregable (solo QA-018).
