...
Team, Clients y FAQ están registrados en `ce_construction_home_sections()` desde el Entregable UX-1.1, pero **no forman parte de `ce_construction_default_home_order()`**. 🆕 **Sprint UX-2, Entregable UX-2.1:** `template-parts/team.php` y `template-parts/clients.php` ya existen (reutilizan `content-equipo.php`/`content-cliente.php` como partial de card, mismo patrón que `projects.php` — ver la fila correspondiente en la sección 3). `template-parts/faq.php` sigue pendiente del Entregable UX-2.2. Que team/clients ya tengan template-part no las activa automáticamente en el Home: `CE_Customize_Home_Sections_Control` (`inc/customizer.php`, UX-1.2) verifica su disponibilidad en tiempo de render con `file_exists()` — al existir ya el archivo, su casilla del panel "CE: Home Builder" deja de forzarse deshabilitada y de mostrar "(próximamente)" automáticamente, sin que este Entregable haya necesitado modificar `inc/customizer.php`; pero el administrador debe activarlas explícitamente (casilla) para que aparezcan en el Home publicado. Ver `DECISIONS.md` D-047.
