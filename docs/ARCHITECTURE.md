...
| `team.php`, `clients.php` | `front-page.php` (vía Home Builder — `inc/home-builder.php`, claves `team`/`clients`) | 🆕 Sprint UX-2, Entregable UX-2.1. Secciones de Home para Equipo/Clientes; reutilizan `content-equipo.php`/`content-cliente.php` como partial de card dentro de un `WP_Query` acotado, mismo patrón que `projects.php`. No auto-activadas por defecto: requieren activación explícita del administrador en el panel "CE: Home Builder" del Customizer (ver `DECISIONS.md` D-047). |
| `faq.php` | `front-page.php` (vía Home Builder — `inc/home-builder.php`, clave `faq`) | 🆕 Sprint UX-2, Entregable UX-2.2. Sección de Home para Preguntas Frecuentes; mismo patrón que `team.php`/`clients.php`. No auto-activada por defecto (ver `DECISIONS.md` D-048). |
| `content-faq-accordion.php` | `single-servicio.php` (bloque "FAQ relacionadas", desde el Sprint 3) y `faq.php` (Home, desde UX-2.2) | 🆕 Sprint UX-2, Entregable UX-2.2. Partial de **ítem individual** de accordion FAQ, extraído de `single-servicio.php` para evitar duplicación de markup entre ambos contextos — mismo patrón de invocación que `content-servicio.php`/`content-proyecto.php`/`content-equipo.php`/`content-cliente.php` (debe invocarse dentro de un loop y dentro de un `.ce-accordion` ya abierto por el llamador). Ver `DECISIONS.md` D-048. |
...
Team, Clients y FAQ están registrados en `ce_construction_home_sections()` desde el Entregable UX-1.1, pero **no forman parte de `ce_construction_default_home_order()`**. 🆕 **Sprint UX-2 (Entregables UX-2.1 y UX-2.2, COMPLETADO):** `template-parts/team.php`, `template-parts/clients.php` y `template-parts/faq.php` ya existen — las 13 secciones del catálogo tienen template-part real (ver las filas correspondientes en la sección 3). `faq.php` reutiliza además `content-faq-accordion.php`, un partial de ítem compartido con el bloque "FAQ relacionadas" de `single-servicio.php` (existente desde el Sprint 3), extraído en UX-2.2 para no duplicar ese markup. Que las 3 ya tengan template-part no las activa automáticamente en el Home: `CE_Customize_Home_Sections_Control` (`inc/customizer.php`, UX-1.2) verifica su disponibilidad en tiempo de render con `file_exists()` — al existir ya cada archivo, su casilla del panel "CE: Home Builder" deja de forzarse deshabilitada y de mostrar "(próximamente)" automáticamente, sin que ninguno de los dos Entregables de este Sprint haya necesitado modificar `inc/customizer.php`; pero el administrador debe activarlas explícitamente (casilla) para que aparezcan en el Home publicado. Ver `DECISIONS.md` D-047 y D-048.
...
- **v0.8.4 (Sprint UX-2, Entregable UX-2.1 — fase "Optimización UX / Conversión", paralela al Sprint 8 pausado):** dos nuevas secciones de Home. `template-parts/team.php` y `template-parts/clients.php` (sección 3, nueva fila de la tabla de `template-parts/`), reutilizando `content-equipo.php`/`content-cliente.php` como partial de card, mismo patrón que `projects.php` (sección 6, nota actualizada). `inc/home-builder.php` solo recibió actualización de comentarios (sus entradas `team`/`clients` dejan de describir rutas inexistentes) — ninguna de sus 3 funciones cambió de comportamiento. `inc/customizer.php` **no se modificó**: el `file_exists()` ya implementado en UX-1.2 (`CE_Customize_Home_Sections_Control`) resuelve la disponibilidad de forma genérica, así que las casillas de Team/Clients dejan de forzarse deshabilitadas automáticamente al existir ya sus archivos. Ver `DECISIONS.md` D-047 y `docs/CURRENT_UX_SPRINT.md`.
- **v0.8.5 (Sprint UX-2, Entregable UX-2.2 — fase "Optimización UX / Conversión", paralela al Sprint 8 pausado; Sprint UX-2 COMPLETADO):** última sección de Home pendiente. `template-parts/faq.php` (sección 3, nueva fila) y `template-parts/content-faq-accordion.php` (sección 3, nueva fila — partial de ítem compartido con `single-servicio.php`, sección 6 nota actualizada). `single-servicio.php` recibió un único cambio quirúrgico: el bloque interno del accordion FAQ ahora invoca el partial compartido en vez de imprimir el markup inline (verificado por diff como el único bloque distinto del archivo). `inc/home-builder.php` solo recibió actualización de comentarios (entrada `faq` y docblock de `ce_construction_default_home_order()`) — ninguna de sus 3 funciones cambió de comportamiento. `inc/customizer.php` **no se modificó** (mismo mecanismo genérico de `file_exists()` que en UX-2.1). Ver `DECISIONS.md` D-048 y `docs/CURRENT_UX_SPRINT.md`.
