...
#### 🆕 Sprint UX-10 — "Página de Testimonios: CPT propio + Google Reviews (Trustindex embebido tal cual, sección independiente)" — 🟢 **En curso: UX-10.1+UX-10.2 aprobados; UX-10.3/UX-10.4 desbloqueados conceptualmente con alcance corregido (ver D-075), pendiente de tu aprobación explícita de ese nuevo alcance.**
...
**⚠️ Nota de secuencia (ver `DECISIONS.md` D-072 para el análisis completo):** UX-7.8 ("Testimonio en video", todavía sin iniciar) toca los mismos 3 archivos que este Sprint necesita modificar (`inc/cpt-testimonios.php`, `template-parts/testimonials.php`, `content-testimonio-card.php`). **El usuario confirmó explícitamente la Opción 2 de D-072**: Sprint UX-10 se ejecuta ahora, en paralelo, con el Sprint UX-7 pausado en el punto exacto donde está (UX-7.7 aprobado); UX-7 se retoma en UX-7.8 solo después de que UX-10 esté completo. Hasta ahora (UX-10.1/UX-10.2), ninguno de los 2 Sprints tocó `inc/cpt-testimonios.php` ni `content-testimonio-card.php` — el conflicto de archivos compartidos sigue vigente y deberá revisarse cuando se retome UX-7.8.

**🆕 Actualización del estado de "UX-10 completo" (ver `DECISIONS.md` D-075):** el bloqueo de D-074 (Trustindex sin API pública) llevó a revisar la premisa funcional de UX-10.3/UX-10.4 — el usuario confirmó que "mismo componente visual para ambas fuentes" nunca fue un requisito independiente. Nuevo alcance aprobado conceptualmente: Trustindex se embebe tal cual como sección independiente del Home Builder (`google_reviews`), sin normalizar datos; la combinatoria de fuentes (antes UX-10.4) queda absorbida por el propio Home Builder. UX-10.3/UX-10.4 **no inician sin la aprobación explícita del usuario de este alcance corregido** (D-038) — con esa aprobación, el Sprint UX-10 queda completo y la condición "UX-7 se retoma en UX-7.8 solo después de que UX-10 esté completo" pasaría a cumplirse.

**Dependencias:** ninguna sobre el sistema de cotización, el Home Builder existente (fuera del registro aditivo estándar) o el Sprint 8 pausado. Proveedor ya confirmado (Trustindex, ver nota más abajo); UX-10.3 solo depende ya de que el administrador configure, en su momento, el código de embed/shortcode real de su cuenta en el campo del Customizer — no bloquea la implementación del mecanismo en sí (la sección se autooculta sin ese dato, mismo criterio que `trust_badges`).

| Entregable | Objetivo | Riesgo | Estado |
|---|---|---|---|
| UX-10.1 | Página dedicada de Testimonios: `page.php` + `[ce_section key="testimonials_full"]` (decisión de diseño confirmada por el usuario — ver abajo), con hero interno (heredado de `page.php`), grid completo con paginación, usando exclusivamente el CPT propio `testimonio` (sin Google todavía). | Bajo — reutiliza patrones ya probados (`archive-clientes.php`, UX-6) | ✅ **Aprobado explícitamente por el usuario.** Protegido explícitamente sin cambios por D-075. Ver `DECISIONS.md` D-073. |
| UX-10.2 | CTA "Ver todos los testimonios" en el teaser del Home (`template-parts/testimonials.php`), enlazando a la página de UX-10.1 | Bajo | ✅ **Aprobado explícitamente por el usuario.** Cerrado junto con UX-10.1 (mismo enlace de destino). Protegido explícitamente sin cambios por D-075. Ver `DECISIONS.md` D-073. |
| UX-10.3 | ~~Integración de Google Reviews (Opción Híbrida C): conexión con Trustindex, normalización al formato de `content-testimonio-card.php`~~ → **Nueva sección independiente `google_reviews` en el Home Builder**: embebe el widget/shortcode de Trustindex tal cual (campo de embed en el Customizer), sin normalizar datos ni tocar `content-testimonio-card.php`. Ver `docs/DECISIONS.md` D-075 (corrige la premisa de D-072/D-073 tras el hallazgo de D-074: Trustindex sin API pública). | Bajo-Medio — sección nueva del Home Builder, sin capa de normalización/caché | 🟢 **Desbloqueado conceptualmente — pendiente de tu aprobación explícita del alcance de D-075 antes de escribir código.** Incluye resolver el saneamiento del campo de embed (D-075, Decisión 3) durante la implementación. |
| UX-10.4 | ~~Selector de fuente en el Customizer (`ce_testimonials_source`: propios / Google / ambos)~~ → **Absorbido por el Home Builder existente**: con `testimonials` y `google_reviews` como claves independientes, activar/desactivar/reordenar desde "CE: Home Builder" resuelve las 4 combinaciones sin mecanismo nuevo. | N/A — sin Entregable propio | 🟢 **Sin backlog pendiente bajo este nombre — ver D-075, Decisión 2.** |

**Decisión de diseño — ✅ confirmada explícitamente por el usuario:** Página de WordPress normal (`page.php`, ya existente desde UX-6.1) + `[ce_section key="testimonials_full"]` (patrón UX-6.2, más flexible para que el administrador agregue contenido adicional alrededor) — **no** `archive-testimonio.php` nativo. `inc/cpt-testimonios.php` no se modifica como consecuencia (`has_archive` permanece `false`). Detalle técnico completo: `DECISIONS.md` D-073.

**Proveedor de Google Reviews (UX-10.3) — ✅ confirmado explícitamente por el usuario:** Trustindex, incorporado como su propio widget/shortcode embebido tal cual (sin normalización de datos — ver `DECISIONS.md` D-075), con el requisito explícito de que la integración sea capaz de utilizar cualquier plan contratado (no atada a un nivel de plan específico). Este dato queda registrado para cuando se inicie UX-10.3; ese Entregable no comienza en este cierre.

**Criterios de aceptación del Sprint completo (revisados — ver `DECISIONS.md` D-075):** (a) la página de Testimonios (UX-10.1) y el Home muestran los testimonios propios del CPT vía `content-testimonio-card.php`, sin cambios; (b) el Home permite, de forma independiente, activar/desactivar/reordenar una nueva sección `google_reviews` que embebe el widget/shortcode de Trustindex tal cual — **sin exigir el mismo componente visual que los testimonios propios**, ya que ambas fuentes se mantienen conceptualmente separadas; (c) las 4 combinaciones (solo CPT / solo Trustindex / ambos / ninguno) quedan resueltas por el Home Builder existente, sin selector de fuente adicional; (d) responsive verificado en los 3 breakpoints estándar del proyecto; (e) por defecto (campo de embed de Trustindex vacío), cero cambio de comportamiento respecto al `testimonials.php`/Home actuales.
