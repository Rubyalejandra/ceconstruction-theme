...
#### 🆕 Sprint UX-10 — "Página de Testimonios: CPT propio + Google Reviews (Opción Híbrida C)" — 🟢 **En curso — secuencia confirmada (Opción 2 de D-072): UX-10 ahora, en paralelo, con el Sprint UX-7 pausado en UX-7.7 aprobado.**
...
|---|---|---|---|
| UX-10.1 | Página dedicada de Testimonios: `page.php` + `[ce_section key="testimonials_full"]` (decisión de diseño confirmada por el usuario — ver abajo), con hero interno (heredado de `page.php`), grid completo con paginación, usando exclusivamente el CPT propio `testimonio` (sin Google todavía). | Bajo — reutiliza patrones ya probados (`archive-clientes.php`, UX-6) | 🟡 **Entregado — pendiente de tu aprobación explícita.** Ver `DECISIONS.md` D-073. |
| UX-10.2 | CTA "Ver todos los testimonios" en el teaser del Home (`template-parts/testimonials.php`), enlazando a la página de UX-10.1 | Bajo | 🟡 **Entregado — pendiente de tu aprobación explícita.** Cerrado junto con UX-10.1 (mismo enlace de destino). Ver `DECISIONS.md` D-073. |
| UX-10.3 | Integración de Google Reviews (Opción Híbrida C): conexión con Trustindex (proveedor confirmado por el usuario — debe soportar cualquier plan contratado), normalización de cada reseña al formato de `content-testimonio-card.php`, cacheo vía `transient` | Medio — primera integración externa del tema | ⬜ No iniciado — **no inicia sin tu aprobación explícita de UX-10.1 + UX-10.2** (instrucción explícita del usuario en el cierre de este Entregable) |
