...
>
> **Nota (Sprint 4 — Módulo Proyectos completado):** ver nueva sección 15 al final. El cliente optó por continuar con este sprint de contenido antes que el sprint de correcciones de QA recomendado; los hallazgos de la sección 14 siguen exactamente igual (sin corregir).
...
- ⬜ QA-001 (🔴 Crítico) — Bypass de validación de tipo de archivo en `inc/quote-form.php` — **pendiente de aprobación**
- ⬜ QA-002 (🟠 Alto) — Archivos adjuntos huérfanos, sin registro como attachment — pendiente de aprobación
- ⬜ QA-003 (🟠 Alto) — Política de retención de datos personales no definida — pendiente de decisión del cliente
- ⬜ QA-004 (🟠 Alto) — Sin rate-limiting en el formulario público — pendiente de aprobación
- ⬜ QA-005 (🟠 Alto) — Fallo de contraste WCAG AA en color secundario sobre blanco — pendiente de aprobación
- ⬜ QA-006 (🟠 Alto) — Sidebar "Footer Columna 1" registrado pero no renderizado — pendiente de aprobación
- ⬜ QA-007 (🟠 Alto) — Falta guardia `wp_is_post_revision()` en `inc/meta-boxes.php` — pendiente de aprobación
- ⬜ QA-008 (🟠 Alto) — `CE_THEME_VERSION` no sincronizada, cache-busting roto — pendiente de aprobación
- ⬜ QA-009 (🟠 Alto) — CPT Servicio sin `page-attributes`, `menu_order` no funcional — pendiente de aprobación (requiere tocar `inc/cpt-servicios.php`, ya implementado)
- ⬜ QA-010 a QA-018 (🟡 Medio, 9 hallazgos) — ver `QA_REPORT.md` — pendientes de aprobación
- ⬜ QA-019 a QA-022 (🟢 Bajo, 4 hallazgos con acción recomendada) — ver `QA_REPORT.md` — pendientes de aprobación
