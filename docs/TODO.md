...
- ✅ 8.1 — Correcciones triviales de bajo riesgo — **aprobado explícitamente por el usuario**: QA-010 ✅, QA-011 ✅, QA-013 🟡 (parcial, aprobado tal cual), QA-014 ✅, QA-015 ✅ (verificado, sin código), QA-017 ✅. Ver `docs/DECISIONS.md` D-095.
- ✅ 8.2 — QA-030 (Alto: `CE_THEME_VERSION`/`style.css` congelados, cache-busting roto) — **aprobado explícitamente por el usuario.** Ver `docs/DECISIONS.md` D-095.
- ✅ 8.3 — QA-031 (Alto: adjuntos de cotización potencialmente accesibles por URL directa) — **aprobado explícitamente por el usuario**, tras pruebas funcionales reales exitosas en entorno Apache/2.4.68 + PHP 8.2.33. Ver `docs/DECISIONS.md` D-096 (implementación) y D-097 (aprobación final).
- ✅ 8.4 — QA-032, QA-033, QA-034 (robustez del formulario de cotización: race condition del rate-limit, archivo huérfano, sin idempotencia) — **aprobado explícitamente por el usuario**, tras pruebas funcionales reales. Ver `docs/DECISIONS.md` D-098 (implementación), D-099 (integración final) y D-101 (aprobación).
- ✅ 8.5 — QA-012 (caché de "relacionados"), QA-016 (script inline de metabox sin dependencia formal), QA-035 (autoplay sin pausa accesible), QA-038 (canonical ausente) — **aprobado explícitamente por el usuario**, tras pruebas funcionales reales. Ver `docs/DECISIONS.md` D-102 (implementación) y D-103 (aprobación).
- ✅ 8.6 — QA-036 (foco de overlays: menú móvil, modales, lightbox) — **aprobado explícitamente por el usuario**, tras pruebas funcionales reales. Ver `docs/DECISIONS.md` D-104 (implementación) y D-105 (aprobación).
- ✅ 8.7 — Hallazgos Bajos: QA-019, QA-020, QA-021, QA-022, QA-037, QA-039, QA-040 — **aprobado explícitamente por el usuario**, tras pruebas funcionales reales. Ver `docs/DECISIONS.md` D-106 (implementación) y D-107 (aprobación). **Con esto, el Sprint 8 queda completo y cerrado.** (QA-043, antes candidato a este Entregable, fue resuelto en Sprint UX-11 — ver `DECISIONS.md` D-087/D-090 — y ya no forma parte del alcance del Sprint 8.)
...
- ✅ **Sprint UX-11 "Hero, Formulario del Hero y Header" — CERRADO Y APROBADO EN SU TOTALIDAD.** Plan base de 6 puntos (D-083 a D-090): (1) corrección del recorte del formulario embebido en el Hero + panel visualmente separado, (2) altura del Hero reducida ~15%, (3) Hero interno usa siempre imagen destacada propia (revierte parte de D-063), (4) posición de imagen configurable por adjunto (verificado que no existe punto focal nativo; alternativa implementada), (5) overlay/gradiente configurable (color/dirección/extensión/intensidad), (6) espaciado del Header corregido (resuelve QA-043). Más 3 ajustes puntuales posteriores, también aprobados: (D-091) expansión progresiva del formulario del Hero, (D-092) opacidad configurable del panel, (D-093) recompactado al salir del formulario. Ver `docs/DECISIONS.md` D-083 a D-094.
- **Conclusión de la auditoría UX de cierre:** con UX-7, UX-10 y UX-11 cerrados por completo, **no queda ningún Sprint ni Entregable UX obligatorio pendiente.** UX-5.2, UX-8 y UX-9 son backlog recomendable/futuro, no bloqueante. El proyecto retoma el Sprint 8 (ver sección 23), con los Entregables 8.1 y 8.2 ya aprobados (`docs/DECISIONS.md` D-095).
...
*(Nota de esta sesión: la frase anterior de este pie de página había quedado desactualizada — describía el Sprint 8 como pausado en el Entregable 8.2. El Sprint 8 avanzó desde entonces hasta el Entregable 8.4 inclusive, ya aprobado (`docs/DECISIONS.md` D-095 a D-101). Se corrige aquí para no dejar una nota stale contradiciendo el resto de este mismo archivo.)*
