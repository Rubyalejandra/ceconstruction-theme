...
Se refinó la metodología permanente de Gestión de Sprints y Entregables (D-030) con una **política de actualización incremental de documentación**: cada documento de control se actualiza únicamente cuando cambia lo que ese documento existe para registrar, en vez de tocarse "por rutina" en cada Entregable. `HANDOFF.md` deja de actualizarse en cada Entregable (solo al cerrar un Sprint completo, una sesión continuable, o ante un cambio de arquitectura importante). Se crea `CURRENT_SPRINT.md`, nuevo documento permanente que absorbe el detalle operativo de corto plazo del Sprint activo. Ver `DECISIONS.md` D-034 y `ARCHITECTURE.md` sección 10 para el detalle completo. No se modificó ningún archivo de código en esta nota.
